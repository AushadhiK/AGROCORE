  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    if (isPostMethod) {
      ...
      if (response.status === 201) {
       setSuccessMessage('Profile created successfully! Center Code: ' + response.data.CenterCode);
      ...
      const response = await axios.put(`https://localhost:44345/api/agrarianCenters/${uppercaseValues.CenterCode}`, uppercaseValues);
      if (response.status === 204) { // 204 indicates successful update
          setSuccessMessage(`Profile updated successfully! Center Code: ${uppercaseValues.CenterCode}`);
          setShowSuccessModal(true);
      } else {
          setSuccessMessage('Please check data and status!');
          setShowSuccessModal(true);
  ...
  const handleFind = async (values, { setFieldValue }) => {
    try {
      ...
      if(data.length > 0 ){
        ...
        if(data.AgrarianCenterModel == null){
         
          isPostMethod = true; 
          setFieldValue('CenterName', data[0].LastName + " - " + data[0].LastName);      
  
          setFieldValue('Email', data[0].Email);
          setFieldValue('Address1', data[0].Address1);
        }
        else{
          ...
          <div className="col-xl-12">
            <div className="card" style={{ marginTop: "30px" }}>
              <div className="card-body">
                <br />
                <h5 style={{ color: "#88BDAE" }}>  AGRARIAN CENTER PROFILE</h5>
